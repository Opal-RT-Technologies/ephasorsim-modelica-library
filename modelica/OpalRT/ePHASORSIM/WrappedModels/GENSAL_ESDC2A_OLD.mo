...
block GENSAL_ESDC2A_OLD
  ...
  OpalRT.NonElectrical.Connector.InputInterfacePin TRIP
    annotation (Placement(transformation(extent={{-6,-6},{6,6}},
        rotation=-90,
        origin={0,100})));
  ModelSets.Old.GENSAL_ESDC2A_OLD modelSet(
      partType = partType,
      IBUS = IBUS,
      ID = ID,
      P_gen = P_gen,
...
equation
  ...
  connect(modelSet.bus0, bus0_PIN2INOUT.p) annotation (Line(points={{14.4,5.6},{
          40,5.6},{40,-40},{52,-40}},  color={0,0,0}));
  connect(modelSet.dVREF, dVREF)
    annotation (Line(points={{-15.48,13.88},{-60,13.88},{-60,100}},
                                                               color={0,0,0}));
  connect(TRIP, modelSet.Trip) annotation (Line(points={{0,100},{0,40},{-28,40},
          {-28,23.6},{0,23.6}},   color={0,0,0}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end GENSAL_ESDC2A_OLD;
