...
model SM_GENROU "Single-machine FMU for GENROU (partType=2)"
  ...
  OpalRT.Electrical.Machine.SynchronousMachine.GENROU machine(
    IBUS = IBUS,
    ID = ID,
    P_gen = P_gen,
    Q_gen = Q_gen,
  ...
  parameter Real ZSOURCE_RE = 0 "Machine source impedance" annotation(Dialog(tab = "GENROU Parameters"));
  ...
  parameter Real S1 = 0.4 "Saturation at 1 p.u." annotation(Dialog(tab = "GENROU Parameters"));
  parameter Real S12 = 0.8 "Saturation at 1.2 p.u." annotation(Dialog(tab = "GENROU Parameters"));
