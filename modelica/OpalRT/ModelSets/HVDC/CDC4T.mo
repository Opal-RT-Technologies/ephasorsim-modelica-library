...
model CDC4T "CDC4T Model of HVDC LCC"
  ...
  OpalRT.Electrical.Branch.HVDC.HVDC_LCC.CDC4T hvdc_lcc1(RDC = RDC, DELTI = DELTI, NBR = NBR, RCR = RCR, XCR = XCR, EBASR = EBASR, TRR = TRR, TAPR = TAPR, NBI = NBI, RCI = RCI, XCI = XCI, EBASI = EBASI, TRI = TRI, TAPI = TAPI, ALFDY = ALFDY, GAMDY = GAMDY, C0 = C0, SB = SB, GAMMX = GAMMX, GAMMN = GAMMN, Vmag_rec = Vmag_rec, Vang_rec = Vang_rec, Vmag_inv = Vmag_inv, Vang_inv = Vang_inv, SETVL = SETVL, MDC = MDC, Tidc = Tidc, Tvdc = Tvdc, C1 = C1, C2 = C2, C3 = C3, V1 = V1, V2 = V2, V3 = V3, RCOMP = RCOMP, VSCHED = VSCHED) annotation(Placement(visible = true, transformation(origin={30,-6},  extent={{-30,-30},
            {30,30}},                                                                                                                                                                                                        rotation = 0)));
equation
  connect(bus0, hvdc_lcc1.rectifp) annotation(Line(points={{106,-30},{60,-30}}));
  connect(bus1, hvdc_lcc1.inverterp) annotation(Line(points={{-106,-28},{-60,-28},
          {-60,-30},{0,-30}}));
  connect(dVSCHED, hvdc_lcc1.d_VSCHED) annotation(Line(points={{-47,5},{-30,5},{
          -30,0},{0,0}}));
  connect(dSETVL, hvdc_lcc1.d_SETVL) annotation(Line(points={{-35,27},{-24,27},{
          -24,18},{0,18}}));
  annotation (Icon(
                graphics={
                  Text(
